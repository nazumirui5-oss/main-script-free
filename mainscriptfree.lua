...
return function(AccessKey)
    ...
    local originalHitboxData = {} -- Menyimpan ukuran asli humanoid musuh
    ...
    -- MEMBUAT CONTENT FRAME MENJADI SCROLLING FRAME (Ditingkatkan ke 860px agar semua item baru muat sepenuhnya)
    ...
    ContentFrame.CanvasSize = UDim2.new(0, 0, 0, 860) -- Ukuran ditingkatkan total
    ...
    local InfJumpWarning = createLabel("WARNING: High ban risk if abused frequently.", UDim2.new(0, 6, 0, 157), UDim2.new(0, 128, 0, 20))
    ...
    -- ==========================================
    -- [[ DETIL INTEGRASI LAYOUT FITUR BARU ]]
    -- ==========================================
    createLine(UDim2.new(0, 6, 0, 405)).Parent = ContentFrame
    createLabel("ADVANCED COMBAT & HITBOX", UDim2.new(0, 6, 0, 411)).Parent = ContentFrame

    local AutoWalkBtn = createBtn("AUTO WALK: OFF", UDim2.new(0, 6, 0, 423), UDim2.new(0, 128, 0, 20)); AutoWalkBtn.Parent = ContentFrame
    local AutoPassBtn = createBtn("AUTO PASS BOMB: OFF", UDim2.new(0, 6, 0, 448), UDim2.new(0, 128, 0, 20)); AutoPassBtn.Parent = ContentFrame

    local HitboxToggleBtn = createBtn("HITBOX: OFF", UDim2.new(0, 6, 0, 473), UDim2.new(0, 62, 0, 20)); HitboxToggleBtn.Parent = ContentFrame
    local HitboxVisualBtn = createBtn("VISUAL: OFF", UDim2.new(0, 72, 0, 473), UDim2.new(0, 62, 0, 20)); HitboxVisualBtn.Parent = ContentFrame

    createLabel("HITBOX SIZE (1-100)", UDim2.new(0, 6, 0, 498)).Parent = ContentFrame
    local HitboxSliderFrame = Instance.new("Frame", ContentFrame)
    HitboxSliderFrame.Size = UDim2.new(0, 128, 0, 12)
    HitboxSliderFrame.Position = UDim2.new(0, 6, 0, 510)
    ...
    HitboxSliderFrame.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            local moveConn; moveConn = UserInputService.InputChanged:Connect(function(move)
                if move.UserInputType == Enum.UserInputType.MouseMovement or move.UserInputType == Enum.UserInputType.Touch then
                    local pos = math.clamp((move.Position.X - HitboxSliderFrame.AbsolutePosition.X) / HitboxSliderFrame.AbsoluteSize.X, 0, 1)
                    _G.HitboxSizeValue = math.floor(1 + (pos * 99))
                    syncHitboxSlider(_G.HitboxSizeValue)
                end
            end)
            UserInputService.InputEnded:Connect(function(ended)
    ...
    -- ========================================================
    createLine(UDim2.new(0, 6, 0, 530)).Parent = ContentFrame
    createLabel("ROTATING CROSSHAIR SETTINGS", UDim2.new(0, 6, 0, 536)).Parent = ContentFrame
    local CrosshairBtn = createBtn("[U] CROSSHAIR: OFF", UDim2.new(0, 6, 0, 548), UDim2.new(0, 128, 0, 20)); CrosshairBtn.Parent = ContentFrame

    local CrosshairScroll = Instance.new("ScrollingFrame", ContentFrame)
    CrosshairScroll.Size = UDim2.new(0, 128, 0, 120)
    CrosshairScroll.Position = UDim2.new(0, 6, 0, 573)
    ...
    -- ========================================================
    createLine(UDim2.new(0, 6, 0, 700)).Parent = ContentFrame
    createLabel("SCREEN RESOLUTION SYSTEM", UDim2.new(0, 6, 0, 706)).Parent = ContentFrame
    local ResBtn = createBtn("[Y] RESOLUTION: OFF", UDim2.new(0, 6, 0, 718), UDim2.new(0, 128, 0, 20)); ResBtn.Parent = ContentFrame

    local ResSliderFrame = Instance.new("Frame", ContentFrame)
    ResSliderFrame.Size = UDim2.new(0, 128, 0, 12)
    ResSliderFrame.Position = UDim2.new(0, 6, 0, 743)
    ...
    createLine(UDim2.new(0, 6, 0, 760)).Parent = ContentFrame
    createLabel("SCALE SETTINGS", UDim2.new(0, 6, 0, 766)).Parent = ContentFrame
    ...
    UIScaleSliderFrame.Position = UDim2.new(0, 6, 0, 778)
    ...
    ExtScaleSliderFrame.Position = UDim2.new(0, 6, 0, 795)
    ...
    -- ========================================================
    -- [[ CUSTOM TWEEN PASS ENGINE ]]
    -- ========================================================
    local isTweeningPass = false
    local function TweenPassToTarget()
        if not lockedTarget or not isAlive(lockedTarget) or isTweeningPass then return end
        local char = LocalPlayer.Character
        ...
        local speed = 260 -- Kecepatan tinggi (hampir instant, namun aman)
        local duration = distance / speed

        hrp.Velocity = Vector3.new(0, 0, 0) -- Hentikan inersia fisik
        
        local targetCFrame = targetHrp.CFrame * CFrame.new(0, 0, 1.5) -- Teleport di depannya sedikit
            ...
            if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
                ...
                if hum and hum.Health > 0 then
                    if _G.HitboxEnabled then
                        if not originalHitboxData[p.UserId] then
                            originalHitboxData[p.UserId] = {
                                Size = hrp.Size,
                        ...
                        local sz = _G.HitboxSizeValue
                        hrp.Size = Vector3.new(sz, sz, sz)
                        hrp.CanCollide = false -- Mencegah bug tabrakan/karakter tersandung
                        ...
                        if originalHitboxData[p.UserId] then
                            local data = originalHitboxData[p.UserId]
                            hrp.Size = data.Size
    ...
    local function ResetAllHitboxes()
        for id, data in pairs(originalHitboxData) do
    ...
    -- Rotasi Vector 2D pada Sumbu Y (Digunakan untuk Wall Check/Obstacle Avoidance)
    local function rotateVectorY(v, angleInDegrees)
        local rad = math.rad(angleInDegrees)
        local c = math.cos(rad)
        local s = math.sin(rad)
        return Vector3.new(v.X * c - v.Z * s, v.Y, v.X * s + v.Z * c).Unit
    end
    ...
    RunService.Heartbeat:Connect(function(dt)
        ...
        -- ========================================================
        -- [[ CORE LOGIC: AUTO WALK & WALL CHECK ]]
        -- ========================================================
        if _G.AutoWalkEnabled and lockedTarget and isAlive(lockedTarget) then
            local targetPos = lockedTarget.Character.HumanoidRootPart.Position
            local toTarget = targetPos - root.Position
            local dist = toTarget.Magnitude

            if amIHolder then
                if dist > 3.5 and not isTweeningPass then
                    local dir = toTarget.Unit
                    local rParams = RaycastParams.new()
                    rParams.FilterDescendantsInstances = {LocalPlayer.Character, Camera}
                    rParams.FilterType = Enum.RaycastFilterType.Exclude

                    -- Melakukan wall check setinggi kaki (-1.0 stud dari RootPart)
                    local obstacleRay = Workspace:Raycast(root.Position + Vector3.new(0, -1, 0), dir * 4, rParams)
                    local finalDir = dir

                    if obstacleRay and obstacleRay.Instance.CanCollide then
                        -- Mencari jalur alternatif dengan merotasikan arah deteksi
                        local left45 = rotateVectorY(dir, 45)
                        local right45 = rotateVectorY(dir, -45)
                        
                        local lRay = Workspace:Raycast(root.Position + Vector3.new(0, -1, 0), left45 * 4, rParams)
                        local rRay = Workspace:Raycast(root.Position + Vector3.new(0, -1, 0), right45 * 4, rParams)
                        ...
                        elseif not rRay then
                            ...
                            local left90 = rotateVectorY(dir, 90)
                            local right90 = rotateVectorY(dir, -90)
                            local lRay90 = Workspace:Raycast(root.Position + Vector3.new(0, -1, 0), left90 * 4, rParams)
                            local rRay90 = Workspace:Raycast(root.Position + Vector3.new(0, -1, 0), right90 * 4, rParams)

                            if not lRay90 then
                                finalDir = left90
                            elseif not rRay90 then
                                finalDir = right90
                            else
                                hum.Jump = true -- Melompati rintangan jika tidak ada celah
                            end
                        end
                    end
                    hum:Move(finalDir)

                    -- Membuat gerakan diam / melayang namun tetap berpindah posisi
                ...
                -- OTOMATIS MUNDUR SETELAH MENGOPER BOM
                local dirAway = (root.Position - targetPos).Unit
                hum:Move(dirAway)
            end
        else
            -- LOGIKA FOLLOW STANDAR (Karakter berjalan biasa)
    ...
    -- ========================================================
    -- [[ 5. TOMBOL EKSTERNAL FREEZE, FLICK, AUTO HOLD, & MANUAL PASS ]]
    -- ========================================================
    ...
    -- TRIGGER UNTUK FITUR BARU CUSTOM REQUEST
    local function toggleAutoWalk()
        _G.AutoWalkEnabled = not _G.AutoWalkEnabled
        AutoWalkBtn.Text = _G.AutoWalkEnabled and "AUTO WALK: ON" or "AUTO WALK: OFF"
    ...
    local function toggleAutoPass()
        ...
        AutoPassBtn.Text = _G.AutoPassEnabled and "AUTO PASS BOMB: ON" or "AUTO PASS BOMB: OFF"
    ...
    local function toggleHitbox()
        ...
        HitboxToggleBtn.Text = _G.HitboxEnabled and "HITBOX: ON" or "HITBOX: OFF"
        HitboxToggleBtn.BackgroundColor3 = _G.HitboxEnabled and _G.AccentColor or Color3.fromRGB(30, 30, 35)
    ...
    local function toggleHitboxVisuals()
        ...
        HitboxVisualBtn.Text = _G.HitboxVisuals and "VISUAL: ON" or "VISUAL: OFF"
        HitboxVisualBtn.BackgroundColor3 = _G.HitboxVisuals and _G.AccentColor or Color3.fromRGB(30, 30, 35)
    end
    ...
    HitboxVisualBtn.MouseButton1Click:Connect(toggleHitboxVisuals)
    ...
    print("Louis Hub FREE V13.5.2: Initialized Successfully with Free Rotating Crosshair (Styles 1-5) & Advanced Combat Suite.")
end
