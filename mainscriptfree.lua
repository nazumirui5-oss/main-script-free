...
return function(AccessKey)
    ...
    -- ==========================================
    -- [[ FITUR USER BARU GLOBALS ]]
    -- ==========================================
    _G.AutoWalkEnabled = false
    _G.AutoWalkRetreatSpeed = 25
    _G.AutoPassEnabled = false
    _G.ManualPassEnabled = false
    _G.PassSpeedValue = 0.1
    _G.HitboxEnabled = false
    _G.HitboxSizeValue = 10
    _G.HitboxVisualEnabled = false
    ...
    ContentFrame.CanvasSize = UDim2.new(0, 0, 0, 1020) -- Diperluas dari 710 agar muat untuk fitur-fitur baru
    ...
    -- FLICK STRENGTH SLIDER (5-90) (FREE / UNLOCKED)
    ...
    local AutoJumpBtn = createBtn("[C] AUTO JUMP: OFF (PREMIUM)", UDim2.new(0, 6, 0, 80), UDim2.new(0, 128, 0, 20)); AutoJumpBtn.Parent = ContentFrame
    ...
    -- [[ CUSTOM CENTER ROTATING CROSSHAIR (FREE INTEGRATION) ]]
    ...
    -- Inisialisasi 15 tombol di scrolling frame
    for i = 1, 15 do
        ...
        -- Proteksi Premium: Gaya 1-5 gratis, Gaya 6-15 dikunci sebagai Premium
        if i > 5 then
            lbl.Text = "Style " .. i .. "\n(PREMIUM)"
    ...
    -- Dynamic External UI Scale Object
    local ExternalUIScale
    local AutoHoldUIScale
    local FlickExternalUIScale
    local PassExternalUIScale -- Tambahan eksternal tween pass scaling
    ...
    -- ========================================================
    -- [[ INTEGRATED AUTO WALK SETTINGS ]]
    -- ========================================================
    createLine(UDim2.new(0, 6, 0, 695)).Parent = ContentFrame
    createLabel("AUTO WALK SETTINGS", UDim2.new(0, 6, 0, 701)).Parent = ContentFrame
    local AutoWalkBtn = createBtn("[T] AUTO WALK: OFF", UDim2.new(0, 6, 0, 713), UDim2.new(0, 128, 0, 20)); AutoWalkBtn.Parent = ContentFrame

    createLabel("RETREAT SPEED (16-50)", UDim2.new(0, 6, 0, 738)).Parent = ContentFrame
    local RetreatSliderFrame = Instance.new("Frame", ContentFrame); RetreatSliderFrame.Size = UDim2.new(0, 128, 0, 12); RetreatSliderFrame.Position = UDim2.new(0, 6, 0, 750); RetreatSliderFrame.BackgroundColor3 = Color3.fromRGB(25, 25, 30); Instance.new("UICorner", RetreatSliderFrame)
    local RetreatSliderFill = Instance.new("Frame", RetreatSliderFrame); RetreatSliderFill.BackgroundColor3 = _G.AccentColor; Instance.new("UICorner", RetreatSliderFill)
    local RetreatSliderText = Instance.new("TextLabel", RetreatSliderFrame); RetreatSliderText.Size = UDim2.new(1, 0, 1, 0); RetreatSliderText.BackgroundTransparency = 1; RetreatSliderText.TextColor3 = Color3.new(1, 1, 1); RetreatSliderText.TextSize = 7; RetreatSliderText.Font = Enum.Font.GothamBold

    local function syncRetreatSlider(val)
        RetreatSliderFill.Size = UDim2.new(math.clamp((val - 16) / 34, 0, 1), 0, 1, 0)
        RetreatSliderText.Text = string.format("RETREAT: %.0f SPEED", val)
    end
    syncRetreatSlider(_G.AutoWalkRetreatSpeed)

    local retreatDragging = false
    RetreatSliderFrame.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            retreatDragging = true
            local percentage = math.clamp((input.Position.X - RetreatSliderFrame.AbsolutePosition.X) / RetreatSliderFrame.AbsoluteSize.X, 0, 1)
            _G.AutoWalkRetreatSpeed = math.floor(16 + (percentage * 34))
            syncRetreatSlider(_G.AutoWalkRetreatSpeed)
        end
    end)

    UserInputService.InputChanged:Connect(function(input)
        if retreatDragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
            local percentage = math.clamp((input.Position.X - RetreatSliderFrame.AbsolutePosition.X) / RetreatSliderFrame.AbsoluteSize.X, 0, 1)
            _G.AutoWalkRetreatSpeed = math.floor(16 + (percentage * 34))
            syncRetreatSlider(_G.AutoWalkRetreatSpeed)
        end
    ...
    UserInputService.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            retreatDragging = false
        end
    end)

    -- ========================================================
    -- [[ INTEGRATED AUTO & MANUAL TWEEN PASS ]]
    -- ========================================================
    createLine(UDim2.new(0, 6, 0, 770)).Parent = ContentFrame
    createLabel("AUTO & MANUAL PASS", UDim2.new(0, 6, 0, 776)).Parent = ContentFrame
    local AutoPassBtn = createBtn("[N] AUTO PASS: OFF", UDim2.new(0, 6, 0, 788), UDim2.new(0, 62, 0, 20)); AutoPassBtn.Parent = ContentFrame
    local ManualPassBtn = createBtn("[M] MANUAL PASS: OFF", UDim2.new(0, 72, 0, 788), UDim2.new(0, 62, 0, 20)); ManualPassBtn.Parent = ContentFrame

    createLabel("TWEEN SPEED (0.05s - 1.0s)", UDim2.new(0, 6, 0, 813)).Parent = ContentFrame
    local TweenSliderFrame = Instance.new("Frame", ContentFrame); TweenSliderFrame.Size = UDim2.new(0, 128, 0, 12); TweenSliderFrame.Position = UDim2.new(0, 6, 0, 825); TweenSliderFrame.BackgroundColor3 = Color3.fromRGB(25, 25, 30); Instance.new("UICorner", TweenSliderFrame)
    local TweenSliderFill = Instance.new("Frame", TweenSliderFrame); TweenSliderFill.BackgroundColor3 = _G.AccentColor; Instance.new("UICorner", TweenSliderFill)
    local TweenSliderText = Instance.new("TextLabel", TweenSliderFrame); TweenSliderText.Size = UDim2.new(1, 0, 1, 0); TweenSliderText.BackgroundTransparency = 1; TweenSliderText.TextColor3 = Color3.new(1, 1, 1); TweenSliderText.TextSize = 7; TweenSliderText.Font = Enum.Font.GothamBold

    local function syncTweenSlider(val)
        TweenSliderFill.Size = UDim2.new(math.clamp((val - 0.05) / 0.95, 0, 1), 0, 1, 0)
        TweenSliderText.Text = string.format("DURATION: %.2fs", val)
    end
    syncTweenSlider(_G.PassSpeedValue)

    local tweenDragging = false
    TweenSliderFrame.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            tweenDragging = true
            local percentage = math.clamp((input.Position.X - TweenSliderFrame.AbsolutePosition.X) / TweenSliderFrame.AbsoluteSize.X, 0, 1)
            _G.PassSpeedValue = 0.05 + (percentage * 0.95)
            syncTweenSlider(_G.PassSpeedValue)
        end
    end)

    UserInputService.InputChanged:Connect(function(input)
        if tweenDragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
            local percentage = math.clamp((input.Position.X - TweenSliderFrame.AbsolutePosition.X) / TweenSliderFrame.AbsoluteSize.X, 0, 1)
            _G.PassSpeedValue = 0.05 + (percentage * 0.95)
            syncTweenSlider(_G.PassSpeedValue)
        end
    ...
    -- ========================================================
    -- [[ INTEGRATED HITBOX EXPANDER ]]
    -- ========================================================
    createLine(UDim2.new(0, 6, 0, 845)).Parent = ContentFrame
    createLabel("HITBOX EXPANDER", UDim2.new(0, 6, 0, 851)).Parent = ContentFrame
    local HitboxBtn = createBtn("[L] HITBOX: OFF", UDim2.new(0, 6, 0, 863), UDim2.new(0, 62, 0, 20)); HitboxBtn.Parent = ContentFrame
    local HitboxVisualBtn = createBtn("VISUAL: OFF", UDim2.new(0, 72, 0, 863), UDim2.new(0, 62, 0, 20)); HitboxVisualBtn.Parent = ContentFrame

    createLabel("HITBOX SIZE (1-100)", UDim2.new(0, 6, 0, 888)).Parent = ContentFrame
    local HitboxSliderFrame = Instance.new("Frame", ContentFrame); HitboxSliderFrame.Size = UDim2.new(0, 128, 0, 12); HitboxSliderFrame.Position = UDim2.new(0, 6, 0, 900); HitboxSliderFrame.BackgroundColor3 = Color3.fromRGB(25, 25, 30); Instance.new("UICorner", HitboxSliderFrame)
    local HitboxSliderFill = Instance.new("Frame", HitboxSliderFrame); HitboxSliderFill.BackgroundColor3 = _G.AccentColor; Instance.new("UICorner", HitboxSliderFill)
    local HitboxSliderText = Instance.new("TextLabel", HitboxSliderFrame); HitboxSliderText.Size = UDim2.new(1, 0, 1, 0); HitboxSliderText.BackgroundTransparency = 1; HitboxSliderText.TextColor3 = Color3.new(1, 1, 1); HitboxSliderText.TextSize = 7; HitboxSliderText.Font = Enum.Font.GothamBold
    ...
    syncHitboxSlider(_G.HitboxSizeValue)
    ...
    HitboxSliderFrame.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            ...
            _G.HitboxSizeValue = math.floor(1 + (percentage * 99))
            syncHitboxSlider(_G.HitboxSizeValue)
        end
    end)
    ...
    -- ==========================================
    -- [[ PATHFINDING WALL CHECK FOR AUTO WALK ]]
    -- ==========================================
    local function findClearPath(startPos, targetPos, rootPart)
        local directDir = (targetPos - startPos).Unit
        local params = RaycastParams.new()
        params.FilterDescendantsInstances = {LocalPlayer.Character, Camera}
        params.FilterType = Enum.RaycastFilterType.Exclude

        -- Deteksi rintangan setinggi kaki (-1.5) dan pinggang (0)
        local heights = {-1.5, 0}
        local rayLength = 6
        local isBlocked = false

        for _, height in ipairs(heights) do
            local origin = startPos + Vector3.new(0, height, 0)
            local hit = workspace:Raycast(origin, directDir * rayLength, params)
        ...
        if not isBlocked then
            return targetPos -- Jalur lurus bersih dari rintangan
        end

        -- Jika terhalang, lakukan pencarian sudut kosong secara menyapu
        local sweepAngles = {30, -30, 60, -60, 90, -90, 120, -120}
        for _, angle in ipairs(sweepAngles) do
            local lookAtCF = CFrame.lookAt(startPos, targetPos)
            local testDir = (lookAtCF * CFrame.Angles(0, math.rad(angle), 0)).LookVector
            local angleBlocked = false

            for _, height in ipairs(heights) do
                local origin = startPos + Vector3.new(0, height, 0)
                local hit = workspace:Raycast(origin, testDir * rayLength, params)
                if hit and hit.Instance.CanCollide then
                    angleBlocked = true
                    break
                end
            ...
            if not angleBlocked then
                return startPos + (testDir * rayLength) -- Kembalikan waypoint jalur kosong terdekat
            end
        end

        return targetPos -- Fallback default jika terkurung sepenuhnya
    end

    -- ==========================================
    -- [[ TWEEN PASS LOGIC ]]
    -- ==========================================
    local isTweeningPass = false
    local function executeTweenPass()
        if isTweeningPass or not lockedTarget or not isAlive(lockedTarget) then return end
        local char = LocalPlayer.Character
        ...
        isTweeningPass = true
        local duration = _G.PassSpeedValue or 0.1
        
        -- CFrame di dekat target (jarak 1.5 stud aman untuk pengoperan)
        local targetCF = tHrp.CFrame * CFrame.new(0, 0, 1.5)
        local tweenInfo = TweenInfo.new(duration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
        local tween = TweenService:Create(hrp, tweenInfo, {CFrame = targetCF})
        tween:Play()
        
        tween.Completed:Connect(function()
            isTweeningPass = false
        end)
    end
    ...
    RunService.Heartbeat:Connect(function(dt)
        ...
        -- Wall Walk Logic Terkait Dihapus Sepenuhnya
        isSticking = false
        ...
        -- LOGIKA AUTO WALK & OBSTACLE AVOIDANCE
        if _G.AutoWalkEnabled then
            if amIHolder then
                -- Karakter diam di tempat secara visual (mematikan script animasi bawaan agar sliding dalam posisi idle)
                pcall(function()
                ...
                if lockedTarget and isAlive(lockedTarget) then
                    ...
                    local targetWaypoint = findClearPath(root.Position, tRoot.Position, root)
                    hum:MoveTo(targetWaypoint)
                end
            else
                -- Baru saja mengoper bom, paksa mundur menggunakan Retreat Speed
                if lastHadBomb then
                    retreatTimer = 3.0 -- Durasi mundur otomatis dalam detik
                end
                ...
                if retreatTimer > 0 and lockedTarget and isAlive(lockedTarget) then
                    ...
                    hum.WalkSpeed = _G.AutoWalkRetreatSpeed or 25
                    hum:MoveTo(root.Position + (retreatDir * 20))
                else
                    -- Kembalikan animasi berjalan normal saat kondisi diam/retreat selesai
                    pcall(function()
                        local anim = LocalPlayer.Character:FindFirstChild("Animate")
            ...
            -- Pastikan script animasi aktif saat Auto Walk dimatikan
            pcall(function()
                local anim = LocalPlayer.Character:FindFirstChild("Animate")
                if anim and anim.Disabled then anim.Disabled = false end
            end)
        ...
        -- LOGIKA AUTO PASS DENGAN TWEEN
        if _G.AutoPassEnabled and amIHolder and lockedTarget and isAlive(lockedTarget) and not isTweeningPass then
            local dist = (root.Position - lockedTarget.Character.HumanoidRootPart.Position).Magnitude
            if dist <= 45 then -- Batas jangkauan trigger auto pass aman
                executeTweenPass()
            end
        end

        -- LOGIKA FOLLOW NORMAL (Hanya bekerja jika Auto Walk dimatikan)
        if not _G.AutoWalkEnabled and lockedTarget and isAlive(lockedTarget) then
        ...
        elseif not _G.AutoWalkEnabled then 
            hum.WalkSpeed = 16 
        end

        if lastHadBomb and not amIHolder and not _G.AutoWalkEnabled then 
                    ...
                    if p ~= LocalPlayer and p.Character then
                        ...
                        if hrp then
                            if _G.HitboxEnabled then
                                local sz = _G.HitboxSizeValue or 10
                                hrp.Size = Vector3.new(sz, sz, sz)
                                hrp.CanCollide = false
                                
                                local selectionBox = hrp:FindFirstChild("HitboxVisualBox")
                                if _G.HitboxVisualEnabled then
                                    if not selectionBox then
                                        selectionBox = Instance.new("SelectionBox")
                                        selectionBox.Name = "HitboxVisualBox"
                                        selectionBox.Color3 = _G.AccentColor
    ...
    -- INTEGRASI WALLHOP & INFINITE JUMP
    UserInputService.JumpRequest:Connect(function()
        ...
        if _G.InfJumpEnabled and not jumpDebounce then
            ...
            if hum then
                ...
                task.spawn(function()
                    task.wait(0.2) -- Debounce untuk mencegah penumpukan tinggi lompatan
                    jumpDebounce = false
                end)
            end
        end
    ...
    -- ========================================================
    -- [[ 5. TOMBOL EKSTERNAL FREEZE (LAG), FLICK, AUTO HOLD, & TWEEN PASS ]]
    -- ========================================================
    ...
    -- UIScale objek untuk tombol eksternal agar membesar proporsional
    ExternalUIScale = Instance.new("UIScale", FreezeExternalBtn)
    ExternalUIScale.Scale = 1.0

    -- Tombol Memberhentikan Freeze
    ...
    StopFreezeExternalBtn.Position = UDim2.new(0, 0, 1, 5) -- Di bawah tombol Freeze dengan jarak 5 pixel
    ...
    StopFreezeStroke.Color = Color3.fromRGB(255, 50, 50) -- Indikator Merah untuk menghentikan
    ...
    -- TOMBOL TWEEN PASS BOMB MANUAL (EKSTERNAL)
    ...
    PassExternalBtn.Position = UDim2.new(0.5, 125, 0.8, 0)
    PassExternalBtn.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
    PassExternalBtn.Text = "TWEEN PASS"
    ...
    -- Tombol Freeze Draggable (Terhubung dengan Lock/Unlock 'isLocked')
    ...
    FreezeExternalBtn.MouseButton1Click:Connect(function()
        ...
        -- Simulasi lag (anchor karakter di tempat)
        if char then
        ...
        -- Tunggu maksimal 3.5 detik atau sampai dihentikan secara manual
        local elapsed = 0
    ...
    -- Interaksi Tombol Eksternal Pass Manual (Tween Pass)
    PassExternalBtn.MouseButton1Click:Connect(function()
        if lockedTarget and isAlive(lockedTarget) then
            PassExternalBtn.Text = "PASSING..."
            PassExternalBtn.BackgroundColor3 = Color3.fromRGB(0, 200, 100)
            executeTweenPass()
    ...
    local function toggleAutoPass()
        ...
        AutoPassBtn.Text = _G.AutoPassEnabled and "[N] AUTO PASS: ON" or "[N] AUTO PASS: OFF"
        AutoPassBtn.BackgroundColor3 = _G.AutoPassEnabled and _G.AccentColor or Color3.fromRGB(30, 30, 35)
    end

    local function toggleManualPass()
        _G.ManualPassEnabled = not _G.ManualPassEnabled
        ManualPassBtn.Text = _G.ManualPassEnabled and "[M] MANUAL: ON" or "[M] MANUAL: OFF"
        ManualPassBtn.BackgroundColor3 = _G.ManualPassEnabled and _G.AccentColor or Color3.fromRGB(30, 30, 35)
        PassExternalBtn.Visible = _G.ManualPassEnabled
    end

    local function toggleHitbox()
        _G.HitboxEnabled = not _G.HitboxEnabled
        HitboxBtn.Text = _G.HitboxEnabled and "[L] HITBOX: ON" or "[L] HITBOX: OFF"
    ...
    local function toggleHitboxVisual()
        _G.HitboxVisualEnabled = not _G.HitboxVisualEnabled
        HitboxVisualBtn.Text = _G.HitboxVisualEnabled and "VISUAL: ON" or "VISUAL: OFF"
        HitboxVisualBtn.BackgroundColor3 = _G.HitboxVisualEnabled and _G.AccentColor or Color3.fromRGB(30, 30, 35)
    end
    ...
    UserInputService.InputBegan:Connect(function(input, gameProcessed)
        ...
        elseif key == Enum.KeyCode.T then toggleAutoWalk() -- Keybind Auto Walk
        elseif key == Enum.KeyCode.N then toggleAutoPass() -- Keybind Auto Pass
        elseif key == Enum.KeyCode.M then toggleManualPass() -- Keybind Manual Pass
        elseif key == Enum.KeyCode.L then toggleHitbox() -- Keybind Hitbox
    ...
    print("Louis Hub FREE V13.5.2: Initialized Successfully with Free Rotating Crosshair (Styles 1-5) & Screen Resolution System.")
end
